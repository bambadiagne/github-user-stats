...
def fetchAllSenegalese(query):
    ...
    all_users.extend(user for user in data['nodes'] if(user!={}))#remove empty json
    ...
    return {'users':all_users,'userCount':userCount,'message':"SUCCESSFUL"}    
def user_fetcher(query=None,variables=None,single_fetch=False):
    ...
    remaining_requests=math.ceil(fetched_result['userCount']/MAX_GITHUB_FETCH)
    for i in range(remaining_requests):
      users=fetchAllSenegalese(LOCATIONS_FOR_SENEGAL+f" sort:joined created:<{last_joined_user['createdAt']}")
  ...
  for node in user['repositories']['edges']:
      for language in node['node']['languages']['edges']:
        ...
        if(languages_dict.get(language_type)):
          ...
          languages_dict[language_type]=language['size']
  user['most_used_anguages']=list(map(lambda x:x[0],sorted(languages_dict.items(), key=lambda item: item[1],reverse=True)[:5])) 
  user['most_starred_repo']=user['repositories']['edges'][0]['node'] if(user['repositories']['edges']) else None
